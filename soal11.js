...
// 1. buat function graceApple
function graceApple(user){
    // 2. buat variable result berupa array kosong yg nantinya utk menyimpan data user yang memiliki teman grace
    let result = []
    // 3. buat variable result berupa array kosong yg nantinya utk menyimpan data user yang memiliki teman grace dan faforite buah apple dan banana
    let result2 = []

    // 4. looping cek semua data 1 1
    for(let i = 0; i < user.length; i++){
        // 4. for of untuk mengecek data array di key friend
        for(const friend of user[i].friends){
            // 5. filter untuk frind yang bernama grace 
            if(friend.name === "Grace"){
                // 6. push datanya ke variable result
                result.push(user[i])
            }
        }
    }
    
    // 7. Looping cek semua data di variable result 1 1 
    for(let j = 0; j < result.length; j++){
        // 8. filter untuk user yang faforite buahnya apple atau banana
        if(result[j].favoriteFruit === "apple" || result[j].favoriteFruit === "banana"){
            // 9. push datanya ke variable result2
