...
// 1. buat function companyE
function companyE(letter){
    // 2. buat variable utk menampung hasil dari data company yang sudah di split
    const companySplit = []
    // 3. buat variable utk menampung hasil dari data company yang sudah di filter huruf belakangya E
    ...
    for(let j = 0; j < companySplit.length; j++){
        // 9. buat variable yang ber isi index terakrih atau yang paling kanan dari companySplit
        const getLastIndex = companySplit[j].length - 1
        // 10. buat filter untuk yang huruf belakangya sesuai argumen
        if(companySplit[j][getLastIndex] == letter){
            // 12. jika ya, push datanya ke companySPlitFIlter
            companySplitFilter.push(users[j])
        }
    }

    // 12. print companySPlitFIlter
    console.log(companySplitFilter)
    // 13. balikan data untk mendapatkan jumlah usernya
    return `Jumlah user company yang huruf belakangnya E ada ${companySplitFilter.length}`
}

console.log(companyE("E"))