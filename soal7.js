...
// 1. buat function noIrfiAdella
function noIrfiAdella(user){
    // 2. buat variable result yg nantinya utk menyimpan data
    let result = []
    // 3. looping utk cek semua data 1 1
    for(let i = 0; i < user.length; i++){
        // 4. buat variable isFriend false
        let isFriend = false
        // 5. for of utk mengecek data ke key friend
        for(const friend of user[i].friends){
            // 6. buat filter jika frined memiliki teman irfi atau adella
            if(friend.name === "Irfi" || friend.name === "Adella"){
                // 7. jika iya, is friend ubah menjadi true
                isFriend = true
            }
        }
        // 8. buat filter utk tidak sama dengan isFriend 
        if(!isFriend){
            // 9. push datanya ke result
            result.push(user[i])
        }
    }
...
console.log(noIrfiAdella(users))