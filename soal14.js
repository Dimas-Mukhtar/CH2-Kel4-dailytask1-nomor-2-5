...
// 1. buat function changeEmail
function changeEmail(user){
    // 2. buat variable result yg nantinya utk menyimpan data
    let result = []
    // 3. looping utk cek semua data 1 1
    for(let i = 0; i < user.length; i++){
        // 4. split email user dengan "@", lalu disimpan di variable split
        const split = user[i].email.split("@")

        // 5. buat pengkondisian jika panjang split adalah 2
        if(split.length === 2){
            // 6. ambil index ke 0 dari split, lalu disimpan di getUsername
            const getUsername = split[0]
            // 7. lalu definisikan email user dengan variable getUsename dan text @binar.org
            user[i].email = `${getUsername}@binar.org`
