...
// 1. buat function getUser berparameter genders dan eyeColor
function getUser(genders, eyeColor){
    // 2. buat variable userFilter berupa array kosong yg nantinya utk menyimpan data
    let userFilter = []
    // 3. looping utk cek semua data 1 1
    for(let i = 0; i < users.length; i++){
        // 4. buat filter utk user gender dan eyeColor sesuai argumen isian 
        if(users[i].gender == genders || users[i].eyeColor == eyeColor){
            // 5. push datanya ke userFilter
            userFilter.push(users[i])
        }
    }

    // 6. print userFilter utk melihat hasil
    console.log(userFilter)
    // 7. return length userFilter utk print jumlah user yg sesuai filter
    return `Jumlah user yang bergender female atau brown ada ${userFilter.length}`
}

console.log(getUser("female", "brown"))