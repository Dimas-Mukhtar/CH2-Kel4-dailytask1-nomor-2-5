...
// 1. buat function getKarakter
function getKarakter(user){

    // 2. buat variable result berupa array kosong yg nantinya utk menyimpan data yang sudah difilter
    let result = []
    // 3. looping cek semua data 1 1
    for(let i = 0; i < user.length; i++){
        // 4. buat filter untuk user karakternya kurang dari 10 karakter dan umur nya diatas 30 atau gender nya Male dan eyeColor nya brown
        if((user[i].name.length < 10 && user[i].age > 30) || (user[i].gender === "female" && user[i].eyeColor === "brown")){
            // 5. push datanya ke variable result
            result.push(user[i])
        }
    }
