...
// 1. buat funciton lautGunung
function lautGunung(user){
    // 2. buat variable result yg nantinya utk menampung data
    let result = []

    // 3. looping utk cek semua data 1 1
    for(let i = 0; i < user.length; i++){
        // 4. buat variable laut yg isinya "laut"
        let laut = "laut"
        // 5. buat variable gunung yg isinya "gunung"
        let gunung = "gunung"
        // 6. pengkondisian jika gender dari user female 
        if(user[i].gender === "female"){
            // 7. jika iya, ubah addressnya menjadi laut
            user[i].address = laut
            // 8. push datanya ke result
            result.push(user[i])
            // 9. pengkondisian jika gender dari user male
        } else if(user[i].gender === "male"){
            // 10. jika iya, ubah addressnya menjadi gunung
            user[i].address = gunung
            // 11. push datanya ke result
            result.push(user[i])
        }
    }
    // 12. mengembalikan data
    return result
}
