...
// 1. buat function swap
function swap(user){
    // 2. buat variable result berupa array kosong yg nantinya utk menyimpan data
    let result = []

    // 3. looping cek semua data 1 1
    for(let i = 0; i < user.length; i++){
        // 4. mengambil semua data name dari user, lalu di simpan di variable fullName
        const fullName = user[i].name
        // 5. memecah name dengan spasi jadi ada 2 length dimana index 0 itu nama depan, index 1 itu nama belakang. lalu di simpan di variable pecah
        const pecah = fullName.split(" ")
        // 6. mengambil nama depan yang berada di index ke 0, lalu di simpan di variable firstName
        const firstName = pecah[0]
        // 7. mengambil nama belakang yang berada di index ke 1, lalu di simpan di variable lastName
        const lastName = pecah[1]
        // 8. menukar posisi variable firstName yang berisi nama depan dan lastName yang berisi nama belakang, lalu di inisiasikan ke data user name
        user[i].name = `${lastName} ${firstName}`
        // 9. push data ke variable result
        result.push(user[i])
    }

    // 10. mengembalikan data
    return result
}
